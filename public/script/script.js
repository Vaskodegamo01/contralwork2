$(()=>{
   ...
   getNewsById = (id) => {
    ...
    const PopUpNews =async  (e,id) =>{
        ...
        let form = $(`<div id="form">`);
        let form_id = $(`<form method="post" id="modal_form" action="">`);
        ...
        let input_button = $(`<button id="btn_modal" class="btn btn-primary">Отправить</button>`).click((e)=> FullNewsHandler(e,id));
...
const FullNewsHandler =(e,id) =>{
        let idForm = $("#modal_form");
        if(!idForm[0].checkValidity()){
            $('<input type="submit">').hide().appendTo(idForm).click().remove();
            return;
        }
        e.preventDefault();
       let modalmydiv = $("#modalmydiv");
        const data = new FormData(document.getElementById("modal_form"));
